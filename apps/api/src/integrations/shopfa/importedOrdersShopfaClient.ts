...
export class ImportedOrdersShopfaClient implements ShopfaClient {
  ...
  async getOrderDetailsByNumber(_orderNumber: string): Promise<ShopfaOrderDetails | null> {
    throw ApiError.badRequest(
      "Order details require the Live API data source -- imported order data has no Shopfa admin note or status codes.",
    );
  }
  ...
  async findOrdersByCustomerQuery(_query: string): Promise<ShopfaCustomerOrderRef[]> {
    throw ApiError.badRequest(
      "Order Precheck requires the Live API data source -- imported order data has no Shopfa customer search.",
    );
  }
